...
function cargarCarousel(artistas) {
    artistas.forEach((element)=>{
        $(".carousel-inner").append(`<div id="${element.id}" class="item">
...
function cargarGaleria(artistas) {
    $("#galeria").empty();
    artistas.forEach((element)=>{
        $("#galeria").append(`<a href="artista.html"><img src="${url}images/${element.id}.jpg" class="img-responsive img-galeria" id="galeria-${element.id}" url="${element.id}" title="${element.nombre}" alt="${element.nombre}"/></a>`);
    });
...
function filtrarArtistas(genero) {
    ...
    todosArtistas.forEach((element)=>{
        if (element.generos.split(",").includes(genero)) artistasFiltrados.push(element);
    });
    return artistasFiltrados;
}

//# sourceMappingURL=index.80e6d9ec.js.map
